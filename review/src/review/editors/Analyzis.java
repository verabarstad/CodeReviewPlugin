...
public class Analyzis extends EditorPart implements IMenuListener {
	...
	public static final RGB GOOD_RGB = new RGB(255, 0, 255);
	public static final String ID = "review.editors.Analyzis";
...
//	    if (analyseAll) {	
//	    	analyseResult += Analyse.analyse(0);
////		  ArrayList<CodeFile> aCodeFiles = Database.getCodeFiles("");
////		  for (int i = 0; i < aCodeFiles.size(); i++) {
////			analyseResult += Analyse.parse(aCodeFiles.get(i).getContentString(),aCodeFiles.get(i).getFileId());
////		  }
//	    }
//	    else {
	    	int file = reviewFile.getFileId();
	    	analyseResult += Analyse.analyse(file);
//	    	analyseResult += Analyse.parse(reviewFile.getContentString(),reviewFile.getFileId());
//		}	
		
		document.set(analyseResult);
		//addAnnotations();
	}
	...
	public void init(IEditorSite site, IEditorInput input) 	throws PartInitException {
		...
		setPartName("Analyze "+ reviewFile.getFilename());
	}
	...
	private void hookContextMenu() {
		...
		menuMgr.addMenuListener(new IMenuListener() {
			public void menuAboutToShow(IMenuManager manager) {
				Analyzis.this.fillContextMenu(manager);
			}
		});
